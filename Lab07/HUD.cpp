...
void HUD::Update(float deltaTime)
{
	...
	mCoolDown -= deltaTime;
	if (mCoolDown <= 0.0f && mOwner->GetGame()->GetPlayer()->GetShieldTrack() < 3)
	{
		mOwner->GetGame()->GetPlayer()->GetComponent<HUD>()->DoABarrelRoll();
		mCoolDown = Random::GetFloatRange(15.0f, 25.0f);
	}
}
...
int HUD::GetPlayerHitPoints()
{
	...
	return mOwner->GetGame()->GetPlayer()->GetShieldTrack();
}
