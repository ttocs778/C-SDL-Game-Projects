...
public:
	...
	void CalTeleport(class Portal* entry, class Portal* exit);
...
private:
	...
	bool mFallingByTeleport = false;
	class Portal* mBluePortal = nullptr;
	class Portal* mOrangePortal = nullptr;
	const float FALL_MAX = -750.0f;
	const float TELEPORT_GAP = 0.2f;
	const float VEL_LENGTH_CAP = 350.0f;
	const float LAUNCHING_DISTANCE = 50.0f;
	SoundHandle mFootstepSound;
	SoundHandle mJumpSound;
	SoundHandle mLandSound;
	SoundHandle mTeleportSound;
	SoundHandle mPortalBlueSound;
	SoundHandle mPortalOrangeSound;
	SoundHandle mPortalFailSound;
	SoundHandle mPortalCloseSound;
	MoveState mPreviousState = Falling;
};
