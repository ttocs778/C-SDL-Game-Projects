...
{
	// Return to Idle if the turret has been searching for more than 5 seconds
	if (mStateTimer > 5.0f)
	{
	...
	if (!mHasTarget || mCycleTimer > 1.0f)
	{
		...
		Vector3 forward = Vector3::UnitX * fwdDist;
		Vector3 side = Vector3::UnitY * Random::GetFloatRange(-sideDist, sideDist);
		Vector3 up = Vector3::UnitZ * Random::GetFloatRange(-upDist, upDist);
...
void TurretHead::UpdateFiring(float deltaTime)
{
	...
	{
		targetHealth->TakeDamage(2.5f, GetWorldPosition());
		GetGame()->GetAudio()->PlaySound("Bullet.ogg", false, mAcquiredTarget);
		mFireCooldown = 0.05f;
	}
}
...
void TurretHead::UpdateFalling(float deltaTime)
{
	...
	Vector3 gravity(0.0f, 0.0f, -980.0f); // Downward acceleration
	mFallVelocity += gravity * deltaTime;

	if (mFallVelocity.Length() > 800.0f)
	{
		mFallVelocity.Normalize();
		mFallVelocity *= 800.0f; // Set to terminal velocity
	}
...
void TurretHead::ChangeState(TurretState turretState)
{
	...
	{
		GetGame()->GetAudio()->StopSound(mCurrentVOSound);
	}

	mTurretState = turretState;
	mStateTimer = 0.0f;

	auto iter = mStateSounds.find(turretState);
	if (iter != mStateSounds.end())
	{
		mCurrentVOSound = GetGame()->GetAudio()->PlaySound(iter->second, false, this);
	}
}
...
void TurretHead::TakeDamage()
{
	...
	mCurrentVOSound = GetGame()->GetAudio()->PlaySound("TurretFriendlyFire.ogg", false, this);
}